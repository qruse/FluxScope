...
- **비속어/욕설 절대 금지 및 품격 있는 냉소적 위트 유지 (Zero Profanity & Professional Wit)**:
  - **롱런하는 전문 기술 블로그를 위해 저급한 비속어, 욕설, 과도하게 자극적인 저질 은어는 엄격히 금지함**. (예: `돈지랄`, `개판`, `존나`, `병신`, `아가리`, `호구`, `쥐어패다/후드려패다`, `사기단`, `끔살/총살` 등 절대 사용 금지)
  - 엔지니어의 블랙 유머와 위트는 '욕설이나 비속어'가 아니라, **'이상과 현실의 씁쓸한 괴리, 하드웨어 물리 법칙의 한계, 마케팅 과장 광고와 실제 측정 데이터의 적나라한 대비, 누더기 레거시 코드와 온콜 호출의 애환'을 날카롭게 짚어내는 지적이고 냉소적인 비유**에서 나와야 함 (예: "천문학적인 SiC 부품 원가 폭탄", "1.8GB VRAM 다이어트와 손난로 쓰로틀링", "수만 줄의 if-else 누더기 코드", "새벽 3시 장애 대응 온콜 호출").
