...
- **Prefix 불일치로 인한 캐시 미스 분통**: 프롬프트 빌더에서 dynamic timestamp나 세션 ID를 앞에 넣었다가 캐시 적중률 0% 찍고 요금 폭탄 맞았다는 개발자 경험담 다수
- **TTL 연장 비용 vs 재연산 딜레마**: 1시간 이상 비활성 상태일 때 캐시 스토리지 비용을 계속 낼지, 차라리 필요할 때 웜업할지 트래픽 패턴에 따른 갑론을박 활발함
- **작은 문서에선 RAG가 여전히 경제적**: 10만 토큰 미만 단순 FAQ에서는 캐싱 기본 유지비보다 하이브리드 RAG 벡터 검색이 훨씬 저렴하다는 실무 평가 지배적임
