...
- **ACT vs Diffusion Policy 벤치마크 논쟁**: 액션 청킹 트랜스포머(ACT) 대비 학습 수렴 속도가 빠르고 멀티모달 분포 표현력이 뛰어나다는 평가와, 엣지 보드에서 추론 연산량이 부담된다는 실무자 평가 팽팽함
- **데이터 수집 지옥(Teleoperation Fatigue)**: 30~50회 시연만으로 학습된다고 하지만 그 50회를 완벽한 궤적으로 원격 조종하는 과정 자체가 엄청난 노동력 소모라는 로보틱스 연구원들의 호소
- **배포 시 Jetson 발열 및 전력 스로틀링 이슈**: 로봇 본체에 탑재된 임베디드 NPU에서 50Hz 지속 루프 돌릴 때 서멀 스로틀링으로 인한 프레임 드랍 경험담 다수 공유됨
