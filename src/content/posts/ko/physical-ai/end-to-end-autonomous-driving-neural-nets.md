...
- **블랙박스 디버깅의 악몽**: 모듈형 구조에서는 바운딩 박스 추적기 문제인지 경로 플래너 문제인지 텔레메트리로 명확히 분리되었으나 E2E에서는 왜 멈칫거렸는지 설명 불가능하다는 자율주행 엔지니어들의 고충
- **시뮬레이션-실차 갭(Sim-to-Real Gap)**: 대규모 월드 모델과 합성 데이터로 시뮬레이션 돌려도 비 오는 날 노면 반사나 도로 공사 페인트 같은 롱테일 코너 케이스는 실차에서 여전히 돌발 행동 유발한다는 지적
- **규제 및 법적 책임 공방**: 사고 발생 시 안전 보증 증거를 제출해야 하는 자동차 제조사 입장에서 결정론적 안전 감시자(Safety Arbiter) 없이는 상용 양산 불가하다는 업계 현실
