...
- **말대꾸 타이밍(Interruption) 제어의 난해함**: 사용자가 생각하느라 잠깐 숨 고르는 침묵과 문장 종결을 모델이 구분 못 하고 불쑥 끼어든다는 음성 AI 개발자들의 불만
- **웹소켓/WebRTC 인프라 운영 복잡도**: 기존 stateless HTTP REST API 대비 수만 명의 음성 스트림 동시 접속 세션을 유지하는 오디오 게이트웨이 서버 비용이 만만치 않다는 피드백
- **감정 표현의 비약적 향상 찬사**: 텍스트 번역 후 기계음 TTS로 읽어주던 시절과 비교해 속삭임이나 웃음 섞인 음성 뉘앙스를 네이티브로 재현하는 완성도에 대한 호평
